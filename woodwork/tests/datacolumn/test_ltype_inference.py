...
def test_integer_inference(integers):
    ...
    for series in integers:
        for dtype in dtypes:
            inferred_type = ww.type_sys.infer_logical_type(series.astype(dtype))
...
def test_natural_language_inference_with_threshhold(long_strings):
    ...
    for dtype in dtypes:
        inferred_type = ww.type_sys.infer_logical_type(long_strings[0].astype(dtype))
        assert inferred_type == NaturalLanguage
        inferred_type = ww.type_sys.infer_logical_type(long_strings[1].astype(dtype))
        assert inferred_type == Categorical
...
def test_pdna_inference(pdnas):
    ...
    for index, series in enumerate(pdnas):
        inferred_type = ww.type_sys.infer_logical_type(series)
