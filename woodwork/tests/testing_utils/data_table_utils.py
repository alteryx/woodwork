...
def validate_subset_dt(subset_dt, dt):
    ...
    for subset_col_name, subset_col in subset_dt.columns.items():
        ...
        assert to_pandas(subset_col.to_pandas()).equals(to_pandas(col.to_pandas()))
...
def to_pandas(df, index=None, sort_index=False, int_index=False):
    ...
    # Commenting out for now - will need eventually as we continue Dask/Featuretools integration
    # if index:
