...
def test_to_csv(sample_df, tmpdir):
    ...
    pd.testing.assert_frame_equal(deserialized_df, sample_df)
...
def test_to_disk_with_latlong(latlong_df, tmpdir, file_format):
    ...
    pd.testing.assert_frame_equal(latlong_df, deserialized_df)
...
def test_to_disk_parquet_no_file_extension(sample_df, tmpdir):
    ...
    # Without specifying format, WW doesn't know what type of file this is
...
def test_serialize_s3_pickle(sample_df, s3_client, s3_bucket, profile_name):
    sample_df.ww.init()
    sample_df.ww.to_disk(TEST_S3_URL, format="pickle", profile_name=profile_name)
    make_public(s3_client, s3_bucket)
    deserialized_df = read_woodwork_table(TEST_S3_URL, profile_name=profile_name)
...
@pytest.mark.parametrize("profile_name", [None, False])
def test_deserialize_url_csv(sample_df, profile_name):
    sample_df.ww.init(index="id")
