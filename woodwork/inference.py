...
from woodwork.utils import col_is_datetime
...
def integer_func(series):
    ...
    if (pdtypes.is_integer_dtype(series.dtype) and
            not _is_numeric_categorical(series, numeric_categorical_threshold) and
            any(series.dropna() < 0)):
        return True
    return False


def whole_number_func(series):
    numeric_categorical_threshold = ww.config.get_option('numeric_categorical_threshold')
    if (pdtypes.is_integer_dtype(series.dtype) and
            not _is_numeric_categorical(series, numeric_categorical_threshold) and
            not any(series.dropna() < 0)):
        return True
    return False
